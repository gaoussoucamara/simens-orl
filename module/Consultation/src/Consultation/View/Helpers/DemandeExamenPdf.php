...
class DemandeExamenPdf
{
	...
	protected  function getNoteBio()
	{
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			$Control->convertDate($this->_DonneesPatient['DATE_NAISSANCE']) . "  (" . $this->nbAnnees($this->_DonneesPatient['DATE_NAISSANCE'], $date_actu) . " ans)",
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		...
		while ($this->_yPosition > 70) {
			if ($this->_DonneesDemande && $i == 1) {
				...
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', 'Examens Biologiques:'),
					$this->_leftMargin,
					$this->_yPosition
				);
			...
			if ($i <= $examensBio) {
				...
				if ($this->_DonneesDemande[$i]) {
					...
					$this->_page->drawText(
						$i . ') ' . iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensBioMorpho()[$this->_DonneesDemande[$i]] . ':'),
						$this->_leftMargin,
						$this->_yPosition
					);
			...
			if ($this->_DonneesDemandeMorph && $i == count($this->_DonneesDemande) + 1) {
				...
				$this->_page->setFont($this->_courier, 15);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', 'Examens Morphologiques:'),
					$this->_leftMargin,
					$this->_yPosition
				);
			...
			if ($i > $examensBio && $i <= ($examensBio + $examensMorpho)) {
				...
				if ($this->_DonneesDemandeMorph[$i]) {
					...
					$this->_page->setFont($this->_newTimeGras, 12);
					$this->_page->drawText(
						$n++ . ') ' . iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensBioMorpho()[$this->_DonneesDemandeMorph[$i]] . ':'),
						$this->_leftMargin,
						$this->_yPosition
					);
					$this->_page->setFont($this->_policeContenu, 12);
	...
	public function getPiedPage()
	{
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			'Téléphone: 33 726 25 36   BP: 25 000',
			$this->_leftMargin,
