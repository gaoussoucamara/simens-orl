...
class ConsultationTable {
	...
	public function fetchSexe()
	{
		$adapter = $this->tableGateway->getAdapter ();
		$sql = new Sql($adapter);
		$select = $sql->select('personne');
		$select->columns(array('ID_PERSONNE', 'SEXE'));
		$select->group(array('personne.SEXE'));
		$stat = $sql->prepareStatementForSqlObject($select);
		$result = $stat->execute();
	
		$options = array("Tous");
		foreach ($result as $data) {
			$options[$data['ID_PERSONNE']] = $data['SEXE'];
		}
		return $options;
	}
