...
class TraitementChirurgicalPdf
{
	...
	public function setEnTete()
	{
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			'Polyclinique de l\'UGB de Saint-Louis',
			$this->_leftMargin,
			$this->_pageHeight - 80
		);
		$this->_page->setFont($this->_newTime, 10);
		...
		$this->_page->drawText(
			'Demande de visite pré-anesthésique',
	...
	protected  function getNoteTC()
	{
		...
		$this->_page->drawText(
			$this->_DonneesPatient['NUMERO_DOSSIER'],
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['PRENOM'] . ' ' . $this->_DonneesPatient['NOM']),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		//-----------------------------------------------
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			$Control->convertDate($this->_DonneesPatient['DATE_NAISSANCE']) . "  (" . ($this->_DonneesPatient['AGE']) . " ans)",
			$this->_leftMargin + 240,
			$this->_yPosition
		);
