...
class DemandeExamenMorphoPdf
{
	...
	public function AfficherExamenSurUnePage(){
		...
		//----- -----------------------------------------
		$this->_page->setFont($this->_newTimeGras, 9);
		$this->_page->drawText('ADRESSE (T�l�phone) :',
				$this->_leftMargin+140,
				$this->_yPosition);
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(iconv ('UTF-8' ,'ISO-8859-1' ,$this->_DonneesPatient['ADRESSE'].' ('.$this->_DonneesPatient['TELEPHONE'].')'),
		...
		while($this->_yPosition > 70) {
			...
			if($this->_DonneesDemandeMorph && $i == 1){
				$this->_page->setFont($this->_courier, 15);
				$this->_page->setFont($this->_courier, 15);
				$this->_page->drawText('Analyses Demand�es :',
						$this->_leftMargin,
						$this->_yPosition);
					
				$this->_page->setFont($this->_courier, 15);
				$this->_page->drawText('Renseignement Clinique :',
						$this->_leftMargin +275,
						$this->_yPosition);
		...
		for ($i=0; $i<count($this->_DonneesDemandeMorph); $i++){
			if($this->_DonneesDemandeMorph){
				...
				$this->_page->drawText($this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph[$i]],
						$this->_leftMargin+7,
						$this->_yPosition+380);
				$this->_page->setFont($this->_policeContenu, 12);
				$this->_page->drawText($this->_notesDemandeMorph[$i],
						$this->_leftMargin+275,
						$this->_yPosition+380);
		...
		$this->_page->drawText($this->_DonneesMedecin['prenomMedecin'].' '.$this->_DonneesMedecin['nomMedecin'] ,
				$this->_leftMargin+15, 130);
			
		$this->_page->setFont($this->_policeContenu, 14);
		$this->_page->drawText("Cachet du M�decin :" ,
				$this->_leftMargin+370, 130);
	}
