...
class DemandeExamenMorphoPdf
{
	...
	public function setEnTete()
	{
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			'Service : ' . iconv('UTF-8', 'ISO-8859-1', $this->_Service),
			$this->_leftMargin,
			$this->_pageHeight - 95
		);
	...
	protected  function getNoteMorpho()
	{
		...
		$this->_page->drawText(
			'PRENOM & NOM ( Num̩ro Dossier) :',
			$this->_leftMargin + 85,
			$this->_yPosition
		);
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			$this->_DonneesPatient['PRENOM'] . ' ' . $this->_DonneesPatient['NOM'] . '  (' . $this->_DonneesPatient['NUMERO_DOSSIER'] . ')',
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['SEXE'] . ' (' . $this->_DonneesPatient['AGE'] . "  ans" . ')'),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['ADRESSE'] . ' (' . $this->_DonneesPatient['TELEPHONE'] . ')'),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		...
		//Ligne pour s̩parerles elements
		$this->_page->drawLine(
			$this->_leftMargin,
			$this->_yPosition - 175,
			$this->_pageWidth - 350,
		...
		while ($this->_yPosition > 70) {
			...
			if ($this->_DonneesDemandeMorph && $i == 1) {
				...
				$this->_page->setFont($this->_courier, 15);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', 'Type D\'examen:'),
					$this->_leftMargin,
					$this->_yPosition
				);
				...
				$this->_page->setFont($this->_courier, 15);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', 'Renseignement Clinique :'),
					$this->_leftMargin,
					$this->_yPosition - 150
				);
			...
			if ($i <= ($examensMorpho)) {
				...
				if ($this->_DonneesDemandeMorph[$i]) {
					...
					$this->_page->drawText(
						iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph[$i]]),
						$this->_leftMargin,
						$this->_yPosition - 20
					);
					$this->_page->setFont($this->_policeContenu, 12);
					$this->_page->drawText(
						iconv('UTF-8', 'ISO-8859-1', $this->_notesDemandeMorph[$i]),
						$this->_leftMargin,
						$this->_yPosition - 150
					);
				}
	...
	public function AfficherExamenParPage()
	{
		...
		if ($this->_DonneesDemandeMorph) {
			...
			$this->_page->drawText(
				iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph]),
				$this->_leftMargin,
				$this->_yPosition + 350
			);
			$this->_page->setFont($this->_policeContenu, 12);
			$this->_page->drawText(
				iconv('UTF-8', 'ISO-8859-1', $this->_notesDemandeMorph),
				$this->_leftMargin,
				$this->_yPosition + 250
			);
		}
	...
	public function AfficherExamenSurUnePage()
	{
		$Control = new DateHelper();
		$noteLineHeight = 30;
		$this->_page->setFillColor(new ZendPdf\Color\Html('black')); //Pour le text
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['PRENOM'] . ' ' . $this->_DonneesPatient['NOM'] . '  (' . $this->_DonneesPatient['NUMERO_DOSSIER'] . ')'),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		//-----------------------------------------------
		...
		for ($i = 0; $i < count($this->_DonneesDemandeMorph); $i++) {
			if ($this->_DonneesDemandeMorph) {

				$this->_page->setFont($this->_newTimeGras, 20);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensMorpho()[$this->_DonneesDemandeMorph[$i]]),
					$this->_leftMargin + 7,
					$this->_yPosition + 380
	...
	public function getPiedPage()
	{
		...
		$this->_page->drawText(
			...
			$this->_leftMargin,
			$this->_pageWidth - ($this->_yPosition + 570)
		);
