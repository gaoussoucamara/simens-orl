...
class DemandeExamenBioPdf
{
		...
		while ($this->_yPosition > 70) {
			...
			if ($i <= $examensBio) {
				...
				if ($this->_DonneesDemande[$i]) {
					...
					$this->_page->drawText(
						iconv('UTF-8', 'ISO-8859-1', $this->listeDesExamensBio()[$this->_DonneesDemande[$i]]),
						$this->_leftMargin,
						$this->_yPosition - 20
					);
					$this->_page->setFont($this->_policeContenu, 12);
					$this->_page->drawText(
						iconv('UTF-8', 'ISO-8859-1', $this->_NotesDemande[$i]),
						$this->_leftMargin,
						$this->_yPosition - 150
					);
				}
	...
	public function AfficherExamenParPage()
	{
		...
		if ($this->_DonneesDemande) {
			...
			$this->_page->setFont($this->_newTimeGras, 27);
			$this->_page->drawText(
				$this->listeDesExamensBio()[$this->_DonneesDemande],
				$this->_leftMargin + 50,
				$this->_yPosition + 350
			);
			$this->_page->setFont($this->_policeContenu, 12);
	...
	public function AfficherExamenSurUnePage()
	{
		...
		for ($i = 0; $i < count($this->_DonneesDemande); $i++) {
			if ($this->_DonneesDemande) {
