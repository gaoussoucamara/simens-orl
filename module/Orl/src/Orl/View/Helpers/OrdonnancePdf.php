...
class OrdonnancePdf
{
	...
	protected  function getNoteMedicaments()
	{
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $value['NUMERO_DOSSIER']),
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $value['PRENOM'] . '  ' . $value['NOM']),
			$this->_leftMargin + 210,
			$this->_yPosition
		);
		// 			//-----------------------------------------------
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $value['SEXE']),
			$this->_leftMargin + 210,
			$this->_yPosition
		);
		//-----------------------------------------------
		...
		$this->_page->drawText(
			$Control->convertDate($value['DATE_NAISSANCE']) . "  (" . ($value['AGE']) . " ans)",
			$this->_leftMargin + 210,
			$this->_yPosition
		);
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $value['ADRESSE']),
			$this->_leftMargin + 210,
			$this->_yPosition
		);
		//-----------------------------------------------
		...
		while ($cpt < 14) {
			...
			while ($d < $taille) {
				...
				$this->getNewTime();
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_Medicaments[$d++]),
					$this->_leftMargin + 20,
					$this->_yPosition
				);
				$this->_page->drawText(
				...
				$cpt++;
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_Medicaments[$d++]) . ' ' . iconv('UTF-8', 'ISO-8859-1', $this->_Medicaments[$d++]),
					$this->_leftMargin + 20,
					$this->_yPosition
				);
		...
		//̩
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', 'Cachet et Signature'),
			$this->_leftMargin + 350,
			$this->_yPosition
		);
	}
