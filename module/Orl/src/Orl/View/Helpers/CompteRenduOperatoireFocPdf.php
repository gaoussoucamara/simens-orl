...
class CompteRenduOperatoireFocPdf
{
	protected $_page;
	...
	public function setEnTete()
	{
		$baseUrl = $_SERVER['SCRIPT_FILENAME'];
		$tabURI  = explode('public', $baseUrl);
		$imageHeader = ZendPdf\Image::imageWithPath($tabURI[0] . 'public/img/logoorl.jpg');
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			$this->_DonneesMedecin['prenomMedecin'] . ' ' . $this->_DonneesMedecin['nomMedecin'],
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesDemande['anesthesiste']),
		...
		$this->_page->setFont($this->_newTimeGras, 10);
		$this->_page->drawText(
			'Chech list de séurité:',
			$this->_leftMargin + 300,
			$this->_pageHeight - 110
		);
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $CHL),
		...
		$this->_page->setFont($this->_newTime, 10);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_Service),
			$this->_leftMargin + 40,
			$this->_pageHeight - 95
		);
	...
	protected  function getNoteTC()
	{
		...
		$this->_page->drawText(
			$this->_DonneesPatient['PRENOM'] . ' ' . $this->_DonneesPatient['NOM'],
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['SEXE']),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		...
		if ($date_naissance) {
			...
			$this->_page->drawText(
				$date_naissance . "  (" . $this->_DonneesPatient['AGE'] . " ans)",
				$this->_leftMargin + 240,
				$this->_yPosition
			);
		} else {
		...
		$this->_page->setFont($this->_newTime, 11);
		$this->_page->drawText(
			iconv('UTF-8', 'ISO-8859-1', $this->_DonneesPatient['ADRESSE']),
			$this->_leftMargin + 240,
			$this->_yPosition
		);
		...
		for ($i = 1; $i < 5; $i++) {
			...
			if ($i == 1) {
				...
				$this->_page->setFont($this->_policeContenu, 12);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_DonneesDemande['indication']),
					$this->_leftMargin + 80,
					$this->_yPosition
				);
			} else 
				...
				$this->_page->setFont($this->_newTimeGras, 14);
				$this->_page->drawText(
					'Intervention prï¿½vue :   ',
					$this->_leftMargin,
					$this->_yPosition
				);
				$this->_page->setFont($this->_policeContenu, 12);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_DonneesDemande['intervention_prevue']),
				...
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_DonneesDemande['type_anesthesie']),
				...
				$this->_page->setFont($this->_policeContenu, 13);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $this->_DonneesDemande['numero_vpa']),
					$this->_leftMargin + 60,
					$this->_yPosition
				);
				$noteLineHeight += 15;
		...
		$noteLineHeight -= 15;
		$nbLigne = 0;
		//Gestion du protocole opï¿½ratoire
		//Gestion du protocole opï¿½ratoire

		//Prï¿½paration du texte pour le protocole opï¿½ratoire
		//Prï¿½paration du texte pour le protocole opï¿½ratoire
		...
		$maxLigne = 18;
		//Fin prï¿½paration du texte du protocole opï¿½ratoire
		//Fin prï¿½paration du texte du protocole opï¿½ratoire

		$this->_page->setFont($this->_newTimeGras, 14);
		$this->_page->drawText(
			'Protocole opï¿½ratoire :  ',
			$this->_leftMargin,
			$this->_yPosition
		);
		...
		for ($i = 0; $i < ($maxLigne - $actuLigne) && $i < count($TextProtocoleOperatoire); $i++) {
			...
			$this->_page->setFont($this->_policeContenu, 12);
			$this->_page->drawText(
				iconv('UTF-8', 'ISO-8859-1', $TextProtocoleOperatoire[$i]),
				$this->_leftMargin,
				$this->_yPosition
			);
		...
		//Fin de la gestion du protocole opï¿½ratoire
		//Fin de la gestion du protocole opï¿½ratoire
		...
		//Prï¿½paration du texte des complications
		//Prï¿½paration du texte des complications
		...
		//Fin prï¿½paration du texte des complications
		//Fin prï¿½paration du texte des complications
		...
		if ($complications != "") {
			if ($nbLigne < $maxLigne) {
				...
				for ($i = 0; $i < ($maxLigne - $actuLigne) && $i < count($TextComplication); $i++) {
					...
					$this->_page->setFont($this->_policeContenu, 12);
					$this->_page->drawText(
						iconv('UTF-8', 'ISO-8859-1', $TextComplication[$i]),
						$this->_leftMargin,
						$this->_yPosition
					);
		...
		//Gestion des soins post opï¿½ratoire
		//Gestion des soins post opï¿½ratoire

		//Prï¿½paration du texte des soins post opï¿½ratoire
		//Prï¿½paration du texte des soins post opï¿½ratoire
		...
		//Fin prï¿½paration du texte des soins post opï¿½ratoire
		//Fin prï¿½paration du texte des soins post opï¿½ratoire
		...
		if ($nbLigne < $maxLigne) {
			...
			for ($i = 0; $i < ($maxLigne - $actuLigne) && $i < count($TextSoinsPostOp); $i++) {
				...
				$this->_page->setFont($this->_policeContenu, 12);
				$this->_page->drawText(
					iconv('UTF-8', 'ISO-8859-1', $TextSoinsPostOp[$i]),
					$this->_leftMargin,
					$this->_yPosition
				);
		...
		//Fin Gestion des soins post opï¿½ratoire
		//Fin Gestion des soins post opï¿½ratoire
