			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] != ' ') {
					...
					/*
						...
						$html = "<a class='btn btn-sm btn-success' href='" . $tabURI[0] . "public/secretariat/info-patient/id_patient/" . $aRow[$aColumns[$i]] . "'> ";
			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] != ' ') {
					...
					if ($aColumns[$i] == 'Nom') {
						...
						// $html .= "<a class='btn btn-sm btn-warning' href='".$tabURI[0]."public/urgence/modifier/id_patient/".$aRow[ $aColumns[$i] ]."'> ";
	...
	public function laListePatientsAjax()
	{
		...
		*/
		...
		$sQuery = $sql->select()
			...
			->join(array('pers' => 'personne'), 'pat.ID_PERSONNE = pers.ID_PERSONNE', array('Nom' => 'NOM', 'Prenom' => 'PRENOM', 'Age' => 'AGE', 'Datenaissance' => 'date_naissance',  'Sexe' => 'SEXE', 'Adresse' => 'ADRESSE', 'Nationalite' => 'NATIONALITE_ACTUELLE', 'Taille' => 'TAILLE', 'id' => 'ID_PERSONNE', 'Idpatient' => 'ID_PERSONNE'))
			->order('pat.ID_PERSONNE DESC');
		...
		*/
		...
		/*
		 *liste des patients ayant un rendez-vous 
		 *liste des patients ayant un rendez-vous
		 *liste des patients ayant un rendez-vous
		 */
		$dateDuJour = $date->format('Y-m-d');
		$sql2 = new Sql($db);
		$subselect1 = $sql2->select();
		$subselect1->from(array('rv' => 'rendezvous'));
		$subselect1->columns(array('ID_PATIENT'))->where(array('rv.date' => $dateDuJour));
		$resultat = $sql->prepareStatementForSqlObject($subselect1)->execute();

		$patRV = array();
		foreach ($resultat as $Rvpat) {
			$patRV[] = $Rvpat['ID_PATIENT'];
		}
			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] != ' ') {
					...
					if ($aColumns[$i] == 'Nom') {
						...
						if (in_array($aRow['id'], $patRV)) {
							$row[] = $aRow[$aColumns[$i]] . ' <span style="color:red; font-size: 12px; ">RV</span>';
						} else {
							$row[] = $aRow[$aColumns[$i]];
						}
					} else if ($aColumns[$i] == 'Age') {
			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] != ' ') {
					...
					/*else if ($aColumns[$i] == 'Datenaissance') {
						...
						$html = "<a class='btn btn-sm btn-success' href='" . $tabURI[0] . "public/secretariat/info-programmer-rendez-vous/id_patient/" . $aRow[$aColumns[$i]] . "'> ";
						$html .= '<i class="fa fa-eye" style="color: white;" title="d&eacute;tails"></i> </a>';
