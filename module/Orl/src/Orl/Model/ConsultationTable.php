...
class ConsultationTable
{
	...
	public function listeMedecins($id_service)
	{
		$adapter = $this->tableGateway->getAdapter();
		$sql = new Sql($adapter);
		$select = $sql->select();
		...
		$select->join(array('s' => 'service_employe'), 's.id_employe = e.id_personne');
	...
	/**
	 * RECUPERER LE NOMBRE DES MEDICAMENTS
	 */

	public function boitesMedicaments()
	{
		$adapter = $this->tableGateway->getAdapter();
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->columns(array('*'));
		$select->from(array('boite' => 'boite_medicament'));
	...
	public function getListePatientsConsultesAjax()
	{
		...
		*/
		...
		/*
		 * La liste des patients d�ja consulter par le medecin pr�c�demment
		*/
		...
		*/
		...
		$sQuery = $sql->select()
			...
			->join(array('cons' => 'consultation'), 'cons.id_admission = a.id_admission', array('Id_cons' => 'ID_CONS', "DATEONLY"))
			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] != ' ') {
					...
					/*
						...
						$html = "<a class='btn btn-sm btn-success' href='" . $tabURI[0] . "public/orl/info-patient-recherche?id_patient=" . $aRow['id'] . "'> ";
						$html .= '<i class="fa fa-eye" style="color: white;" title="d&eacute;tails"></i> </a>';

						$html .= "<span style='visibility:hidden;font-size:0;'>" . $Control->convertDate($aRow['DATEONLY']) . "</span>";


						// $html .= "<a class='btn btn-sm btn-warning' href='".$tabURI[0] . "public/orl/info-patient-recherche?id_patient=" . $aRow['id'] ."'> ";
						// $html .='<i class="fa fa-edit" style="color: white;" title="Modifier"></i> </a>';
	...
	public function getListeConsultationPrecedentePatientAjax($id_pat)
	{
		...
		*/
		...
		$sql2 = new Sql($db);
		$sQuery2 = $sql2->select()
			->from(array('cons' => 'consultation'))->columns(array('DATEONLY'))
			->where(array('cons.DATEONLY != ?' => $date, 'cons.ID_PATIENT' => $id_pat))
			->order('cons.id_admission DESC');
		$stat2 = $sql2->prepareStatementForSqlObject($sQuery2)->execute()->current();

		//var_dump(array_values($stat2)); exit();
		...
		*/
		...
		$sQuery = $sql->select()
			...
			->join(array('cons' => 'consultation'), 'cons.id_admission = a.id_admission', array('Id_cons' => 'ID_CONS'))
			->join(array('sd' => 'sous_dossier'), 'sd.id_sous_dossier = cons.id_sous_dossier', array('TypeSousDossier' => 'type_dossier', 'idSousDossier' => 'id_sous_dossier'))
			->where(array('a.date_admis != ?' => $date, 'pat.ID_PERSONNE' => $id_pat /*new In('cons.DATEONLY', array_values($stat2))*/))
			->order('a.id_admission DESC'); //->group('sd.id_sous_dossier');
		...
		*/
		...
		$tabIdDiffSousDossier = array();
		/*
		 * Pr�parer la liste liste des patients � consulter par le medecin
		*/
		foreach ($rResult as $aRow) {
			if (!in_array($aRow['idSousDossier'], $tabIdDiffSousDossier)) {
				$tabIdDiffSousDossier[] = $aRow['idSousDossier'];
					...
					if ($aColumns[$i] != ' ') {
						...
						if ($aColumns[$i] == 'Nom') {
							...
							if ($aRow['idSousDossier'] == 1) {
								...
								$html .= "<img style='margin-right: 15%;' src='" . $tabURI[0] . "public/images_icons/transfert_droite.png' title='d&eacute;tails'></a> </infoBulleVue>";
							} else if ($aRow['idSousDossier'] == 3) {
								$html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/orl/recherche-visualisation-tumeur-parotidienne?id_admission=" . $aRow['id_admission'] . "&id_cons=" . $aRow['Id_cons'] . "'>";
								$html .= "<img style='margin-right: 15%;' src='" . $tabURI[0] . "public/images_icons/transfert_droite.png' title='d&eacute;tails'></a> </infoBulleVue>";
							} else if ($aRow['idSousDossier'] == 4) {
								$html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/orl/recherche-visualisation-corps-etranger?id_admission=" . $aRow['id_admission'] . "&id_cons=" . $aRow['Id_cons'] . "'>";
								$html .= "<img style='margin-right: 15%;' src='" . $tabURI[0] . "public/images_icons/transfert_droite.png' title='d&eacute;tails'></a> </infoBulleVue>";
							} else if ($aRow['idSousDossier'] == 5) {
								$html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/orl/recherche-visualisation-cancer?id_admission=" . $aRow['id_admission'] . "&id_cons=" . $aRow['Id_cons'] . "'>";
								$html .= "<img style='margin-right: 15%;' src='" . $tabURI[0] . "public/images_icons/transfert_droite.png' title='d&eacute;tails'></a> </infoBulleVue>";
							}
	...
	public function addTraitementsInstrumentaux($traitement_instrumental)
	{
		...
		if (
			$traitement_instrumental['endoscopie_interventionnelle'] || $traitement_instrumental['radiologie_interventionnelle'] ||
	...
	//Insertion des demandes d'op�ration
	//Insertion des demandes d'op�ration
