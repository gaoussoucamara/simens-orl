...
class MotifAdmissionTable{
	...
	public function getDonneesPremiereCons($id_pat, $id_sous_dossier){
		$adapter = $this->tableGateway->getAdapter();
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from('consultation');
		$select->columns(array('ID_CONS'));
		$select->where(array('ID_PATIENT'=>$id_pat, 'id_sous_dossier'=>$id_sous_dossier));
		$select->order('DATEONLY DESC');
		$stat = $sql->prepareStatementForSqlObject($select);
		$result = $stat->execute()->current()['ID_CONS'];
