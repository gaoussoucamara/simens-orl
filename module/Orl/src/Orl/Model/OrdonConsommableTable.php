...
class OrdonConsommableTable{
	...
	/**
	 * 
	 * @param $tab : Tableau des m�dicaments
	...
	/**
	 * Ajouter des medicaments dans la base de donn�es
	 */
	...
	public function existeBoites($libelleBoite){
		$adapter = $this->tableGateway->getAdapter ();
		$sql = new Sql ( $adapter );
		$select = $sql->select ();
		$select->columns( array('*'));
		$select->from( array( 'bt' => 'boite_medicament' ));
		$select->where ( array( 'bt.libelleBoite' => $libelleBoite));
	 ...
	 * Ajouter des boites
	 */
	public function addBoites($libelleBoite){
		if( $this->existeBoites($libelleBoite) == false ){
			$db = $this->tableGateway->getAdapter();
			$sql = new Sql($db);
			$sQuery = $sql->insert()
			->into('boite_medicament')
			->values(array('libelleBoite' => $libelleBoite));
			$sql->prepareStatementForSqlObject($sQuery)->execute();
		}
	}
