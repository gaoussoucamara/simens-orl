...
class NotesExamensMorphologiquesTable
{
	...
	public function getNotesExamensMorphologiques($id_cons)
	{
		...
		$tab = array('radio' => '', 'ecographie' => '', 'fibroscopie' => '', 'scanner' => '', 'irm' => '', 'rochers' => '');

		foreach ($result as $donnes) {
			if ($donnes['id_examen'] == 8) {
				$tab['radio']       = $donnes['note'];
			}
			if ($donnes['id_examen'] == 9) {
				$tab['ecographie']  = $donnes['note'];
			}
			if ($donnes['id_examen'] == 10) {
				$tab['fibroscopie'] = $donnes['note'];
			}
			if ($donnes['id_examen'] == 11) {
				$tab['scanner']     = $donnes['note'];
			}
			if ($donnes['id_examen'] == 12) {
				$tab['irm']         = $donnes['note'];
			}
			if ($donnes['id_examen'] == 19) {
				$tab['rochers']         = $donnes['note'];
			}
		}
		return $tab;
	}
