...
use Orl\View\Helpers\CompteRenduOperatoireFocPdf;
...
class OrlController extends AbstractActionController
{
	...
	protected $protocoleOperatoireFocTable;
	...
	protected $motifConsultationTumeurTable;
	protected $peauCervicoFacialeTumeurTable;
	protected $glandeParotidienneTable;
	protected $muqueuseParotideTable;
	protected $pfpParotideTable;
	protected $otoscopieTable;
	protected $examenParacliniqueTumeurTable;
	protected $operationtumeurparotidienneTable;
	protected $examenCliniqueceTable;
	protected $examenParacliniqueceTable;
	protected $operationCorpsEtrangerTable;
	protected $examenCliniquelsoTable;
	protected $motifConsultationlsoTable;
	protected $resteExamenCliniquelsoTable;
	...
	public function getProtocoleOperatoireFoc()
	{
		if (!$this->protocoleOperatoireFocTable) {
			$sm = $this->getServiceLocator();
			$this->protocoleOperatoireFocTable = $sm->get('Orl\Model\ProtocoleOperatoireFocTable');
		}
		return $this->protocoleOperatoireFocTable;
	}
	...
	public function getMotifConsultationTumeur()
	{
		if (!$this->motifConsultationTumeurTable) {
			$sm = $this->getServiceLocator();
			$this->motifConsultationTumeurTable = $sm->get('Orl\Model\MotifConsultationTumeurTable');
		}
		return $this->motifConsultationTumeurTable;
	}

	public function getPeauCervicoFacialeTumeur()
	{
		if (!$this->peauCervicoFacialeTumeurTable) {
			$sm = $this->getServiceLocator();
			$this->peauCervicoFacialeTumeurTable = $sm->get('Orl\Model\PeauCervicoFacialeTumeurTable');
		}
		return $this->peauCervicoFacialeTumeurTable;
	}

	public function getGlandeParotidienne()
	{
		if (!$this->glandeParotidienneTable) {
			$sm = $this->getServiceLocator();
			$this->glandeParotidienneTable = $sm->get('Orl\Model\GlandeParotidienneTable');
		}
		return $this->glandeParotidienneTable;
	}

	public function getMuqueuseParotide()
	{
		if (!$this->muqueuseParotideTable) {
			$sm = $this->getServiceLocator();
			$this->muqueuseParotideTable = $sm->get('Orl\Model\MuqueuseParotideTable');
		}
		return $this->muqueuseParotideTable;
	}

	public function getPfpParotide()
	{
		if (!$this->pfpParotideTable) {
			$sm = $this->getServiceLocator();
			$this->pfpParotideTable = $sm->get('Orl\Model\PfpParotideTable');
		}
		return $this->pfpParotideTable;
	}

	public function getOtoscopie()
	{
		if (!$this->otoscopieTable) {
			$sm = $this->getServiceLocator();
			$this->otoscopieTable = $sm->get('Orl\Model\OtoscopieTable');
		}
		return $this->otoscopieTable;
	}

	public function getExamenParacliniqueTumeur()
	{
		if (!$this->examenParacliniqueTumeurTable) {
			$sm = $this->getServiceLocator();
			$this->examenParacliniqueTumeurTable = $sm->get('Orl\Model\ExamenParacliniqueTumeurTable');
		}
		return $this->examenParacliniqueTumeurTable;
	}

	public function getOperationTumeurParotidienne()
	{
		if (!$this->operationtumeurparotidienneTable) {
			$sm = $this->getServiceLocator();
			$this->operationtumeurparotidienneTable = $sm->get('Orl\Model\OperationTumeurParotidienneTable');
		}
		return $this->operationtumeurparotidienneTable;
	}

	public function getExamenCliniquece()
	{
		if (!$this->examenCliniqueceTable) {
			$sm = $this->getServiceLocator();
			$this->examenCliniqueceTable = $sm->get('Orl\Model\ExamenCliniqueceTable');
		}
		return $this->examenCliniqueceTable;
	}

	public function getExamenParacliniquece()
	{
		if (!$this->examenParacliniqueceTable) {
			$sm = $this->getServiceLocator();
			$this->examenParacliniqueceTable = $sm->get('Orl\Model\ExamenParacliniqueceTable');
		}
		return $this->examenParacliniqueceTable;
	}

	public function getOperationCorpsEtranger()
	{
		if (!$this->operationCorpsEtrangerTable) {
			$sm = $this->getServiceLocator();
			$this->operationCorpsEtrangerTable = $sm->get('Orl\Model\OperationCorpsEtrangerTable');
		}
		return $this->operationCorpsEtrangerTable;
	}


	public function getExamenCliniquelso()
	{
		if (!$this->examenCliniquelsoTable) {
			$sm = $this->getServiceLocator();
			$this->examenCliniquelsoTable = $sm->get('Orl\Model\ExamenCliniquelsoTable');
		}
		return $this->examenCliniquelsoTable;
	}

	public function getMotifConsultationlso()
	{
		if (!$this->motifConsultationlsoTable) {
			$sm = $this->getServiceLocator();
			$this->motifConsultationlsoTable = $sm->get('Orl\Model\MotifConsultationlsoTable');
		}
		return $this->motifConsultationlsoTable;
	}

	public function getResteExamenCliniquelso()
	{
		if (!$this->resteExamenCliniquelsoTable) {
			$sm = $this->getServiceLocator();
			$this->resteExamenCliniquelsoTable = $sm->get('Orl\Model\ResteExamenCliniquelsoTable');
		}
		return $this->resteExamenCliniquelsoTable;
	}
	...
	public function majComplementConsultationAction()
	{
		...
		//modification des ant�c�dents ORL
		//modification des ant�c�dents ORL
		//modification des ant�c�dents ORL
		...
		if ($compteRenduOperatoireOrl) {
			...
			$data['geste'] = $compteRenduOperatoireOrl['geste'];
		...
		if ($leRendezVous) {
			$date_rv = "";
			if ($leRendezVous->date) {
				$date_rv =  $this->controlDate->convertDate($leRendezVous->date);
			}
	...
	public function visualisationConsultationAction()
	{
		...
		//Recuperer les informations sur le surveillant de service pour les Consultations qui diff�rent des Orls prises lors des archives
		$tabInfoSurv = array();
		foreach ($listeConsultation as $listeCons) {
			if ($listeCons['ID_SURVEILLANT']) {
				$tabInfoSurv[$listeCons['ID_CONS']] = $this->getConsultationTable()->getInfosSurveillant($listeCons['ID_SURVEILLANT'])['PRENOM'] . ' ' . $this->getConsultationTable()->getInfosSurveillant($listeCons['ID_SURVEILLANT'])['NOM'];
	...
	public function rechercheVisualisationFicheObservationCliniqueAction()
	{
		...
		$medecin = $this->getConsultationTable()->listeMedecins($IdDuService);
		$form->get('operateur_aide')->setValueOptions($medecin);
		...
		if ($donneesExamenClinique) {
			...
			$data['peau_cervicoFo'] = $donneesExamenClinique['peau_cervicoFo'];
			$data['otoscopieFo'] = $donneesExamenClinique['otoscopieFo'];
			$data['cavite_bucaleFo'] = $donneesExamenClinique['cavite_bucaleFo'];
			$data['fosses_nasalesFo'] = $donneesExamenClinique['fosses_nasalesFo'];
			$data['liFo'] = $donneesExamenClinique['liFo'];
			$data['cat_fo'] = $donneesExamenClinique['cat_fo'];
			$data['reste_examen_cliniqueFo'] = $donneesExamenClinique['reste_examen_cliniqueFo'];
			//$data['tdm_rochers'] = $donneesExamenClinique['tdm_rochers'];
		}

		//Protocole operatoire FOC
		$protocoleOperatoireFoc = $this->getProtocoleOperatoireFoc()->getProtocoleOperatoireFoc($id);
		if ($protocoleOperatoireFoc) {
			$data['anesthesiste'] = $protocoleOperatoireFoc['anesthesiste'];
			$data['indication'] = $protocoleOperatoireFoc['indication'];
			$data['protocole_operatoire'] = $protocoleOperatoireFoc['protocole_operatoire'];
			$data['soins_post_operatoire'] = $protocoleOperatoireFoc['soins_post_operatoire'];
			$data['surveillance'] = $protocoleOperatoireFoc['surveillance'];
		}
		...
		$data['scanner'] = $examen_morphologique['scanner'];
		$data['irm'] = $examen_morphologique['irm'];
		$data['rochers'] = $examen_morphologique['rochers'];
		...
		$listeMedicament = $this->getConsultationTable()->listeDeTousLesMedicaments();
		$listeForme = $this->getConsultationTable()->formesMedicaments();
		$listeBoite = $this->getConsultationTable()->boitesMedicaments();
		$listetypeQuantiteMedicament = $this->getConsultationTable()->typeQuantiteMedicaments();
		...
		if ($donneesDemandeVPA) {
			...
			$data['date_intervenu'] = $donneesDemandeVPA['DATE_INTERVENU'];
			$data['date_sortie'] = $donneesDemandeVPA['DATE_SORTIE'];
			$data['aides'] = $donneesDemandeVPA['AIDES'];
		...
		if ($leRendezVous) {
			$date_rv = "";
			if ($leRendezVous['DATE']) {
				$date_rv = (new DateHelper())->convertDate($leRendezVous['DATE']);
			}
	...
	public function espaceRechercheMedAction()
	{
		...
		//$output = $this->getConsultationTable()->getListePatientsConsultesAjax();
		//var_dump($output); exit(); //nombre al�atoire
		...
		if (isset($_POST['ordonnance'])) {
			...
			for ($i = 1; $i < 10; $i++) {
				...
				if ($nomMedicament == true) {
					...
					$tab[$j++] = $this->params()->fromPost("formess_" . $i);
			...
			if (isset($_POST['protocole_operatoire'])) {
				...
				if (isset($_POST['transfert'])) {
					...
					if (isset($_POST['rendezvous'])) {
						...
						$donneesDemandeRv['dateRv'] = $this->params()->fromPost('date_rv');
						$donneesDemandeRv['heureRV'] = $this->params()->fromPost('heure_rv');
						$donneesDemandeRv['motifRV'] = $this->params()->fromPost('motif_rv');
						$donneesDemandeRv['delaiRV'] = $this->params()->fromPost('delai_rv');
	...
	public function detailInfoLiberationPatientAction()
	{
		...
		if ($nbMedPrescrit != 0) {
			foreach ($listeMedicamentsPrescrits as $Medicaments) {
				...
				$html .= "<script> setTimeout(function(){ $('#formess_'+i).val('" . $Medicaments['BOITE'] . "'); }, 2000);</script>";
	...
	public function imprimerProtocoleOperatoireFAction()
	{
		...
		$donnees['anesthesiste'] = $this->params()->fromPost('anesthesiste');
		$donnees['indication'] = $this->params()->fromPost('indication');
		$donnees['intervention_prevue'] = $this->params()->fromPost('intervention_prevue');
		$donnees['type_anesthesie'] = $this->params()->fromPost('type_anesthesie');
		$donnees['numero_vpa'] = $this->params()->fromPost('numero_vpa');
		$donnees['protocole_operatoire'] = $this->params()->fromPost('protocole_operatoire');
		$donnees['soins_post_operatoire'] = $this->params()->fromPost('soins_post_operatoire');
		...
		//Cr�er la page 1
		$page1 = new CompteRenduOperatoireFocPdf();
	...
	public function updateComplementConsultationAction()
	{
		...
		$this->getSousDossierTable()->addSousDossier(null, $id_cons);
		...
		//Insertion du protocole operatoire FOC
		$this->getProtocoleOperatoireFoc()->deleteProtocoleOperatoireFoc($id_cons);
		$this->getProtocoleOperatoireFoc()->addProtocoleOperatoireFoc($formData, $id_cons, $id_medecin);
		...
		//var_dump($plainteConsultation);exit();
		//Sous Dossier
		...
		$info_examen_morphologique = array(
			...
			'19' => $this->params()->fromPost('rochers_'),

		);
		//var_dump($info_examen_morphologique);
		//exit();
		...
		for ($i = 1; $i < 10; $i++) {
			if ($this->params()->fromPost("medicament_0" . $i)) {
				...
				$boiteMedicament = $this->params()->fromPost("formess_" . $i);
				$info1Posologie = $this->params()->fromPost("nb_medicament_" . $i);
				$info2Posologie = $this->params()->fromPost("quantite_" . $i);

				//var_dump($boiteMedicament.' - '.$info1Posologie.' - '.$info2Posologie);exit();

				if ($nomMedicament) {

					$idMedicaments = $Consommable->getMedicamentByName($nomMedicament)['ID_MATERIEL'];
					if ($idMedicaments) {
						$tab[$j++] = $idMedicaments;
						$tab[$j++] = $formeMedicament;
						$Consommable->addFormes($formeMedicament);
						$tab[$j++] = $boiteMedicament;
						$Consommable->addBoites($boiteMedicament);
						$tab[$j++] = $info1Posologie;
						$tab[$j++] = $info2Posologie;
					} else {
		...
		$infoDemande = array(
			...
			'date_intervenu' => $this->params()->fromPost("date_intervenu"),
			'date_sortie' => $this->params()->fromPost("date_sortie"),
			'aides' => $this->params()->fromPost("aides"),
			'ID_CONS' => $id_cons
		);
		...
		//insertion Motif consultation tumeur
		if ($sous_dossier == 3) {
			$this->getMotifConsultationTumeur()->deleteMotifConsultationTumeur($id_cons);
			$this->getMotifConsultationTumeur()->addMotifConsultationTumeur($formData, $id_cons, $id_medecin);
			//insertion Peau cervico faciale tumeur
			$this->getPeauCervicoFacialeTumeur()->deletePeauCervicoFacialeTumeur($id_cons);
			$this->getPeauCervicoFacialeTumeur()->addPeauCervicoFacialeTumeur($formData, $id_cons, $id_medecin);

			//insertion Glande parotidienne
			$this->getGlandeParotidienne()->deleteGlandeParotidienne($id_cons);
			$this->getGlandeParotidienne()->addGlandeParotidienne($formData, $id_cons, $id_medecin);

			//insertion Muqueuse parotidienne
			$this->getMuqueuseParotide()->deleteMuqueuseParotide($id_cons);
			$this->getMuqueuseParotide()->addMuqueuseParotide($formData, $id_cons, $id_medecin);

			//insertion PFP parotidienne
			$this->getPfpParotide()->deletePfpParotide($id_cons);
			$this->getPfpParotide()->addPfpParotide($formData, $id_cons, $id_medecin);

			//insertion Otoscopie parotidienne
			$this->getOtoscopie()->deleteOtoscopie($id_cons);
			$this->getOtoscopie()->addOtoscopie($formData, $id_cons, $id_medecin);


			//insertion Examen para clinique tumeur parotidienne
			$this->getExamenParacliniqueTumeur()->deleteExamenParacliniqueTumeur($id_cons);
			$this->getExamenParacliniqueTumeur()->addExamenParacliniqueTumeur($formData, $id_cons, $id_medecin);

			//insertion Operation tumeur parotidienne
			$this->getOperationTumeurParotidienne()->deleteOperationTumeurParotidienne($id_cons);
			$this->getOperationTumeurParotidienne()->addOperationTumeurParotidienne($formData, $id_cons, $id_medecin);
			//var_dump('ral');exit();
			$this->getIndicationsOperatoireOrl()->deleteIndicationsOperatoiresOrl($id_cons);
			$this->getIndicationsOperatoireOrl()->addIndicationsOperatoireOrl($formData, $id_cons, $id_medecin);
		}
		...
		if ($sous_dossier == 4) {
			...
			//insertion Examen Clinique Corps Etranger
			$this->getExamenCliniquece()->deleteExamenCliniquece($id_cons);
			$this->getExamenCliniquece()->addExamenCliniquece($formData, $id_cons, $id_medecin);
			//var_dump('sss');
			//exit();
			//insertion Examen Paraclinique Corps Etranger
			$this->getExamenParacliniquece()->deleteExamenParacliniquece($id_cons);
			$this->getExamenParacliniquece()->addExamenParacliniquece($formData, $id_cons, $id_medecin);

			//insertion Operation Corps Etranger
			$this->getOperationCorpsEtranger()->deleteOperationCorpsEtranger($id_cons);
			$this->getOperationCorpsEtranger()->addOperationCorpsEtranger($formData, $id_cons, $id_medecin);
		...
		if ($sous_dossier == 5) {
			//insertion Examen  clinique Lesion Sphere ORL
			$this->getExamenCliniquelso()->deleteExamenCliniquelso($id_cons);
			$this->getExamenCliniquelso()->addExamenCliniquelso($formData, $id_cons, $id_medecin);

			//insertion Motif Consultation Lesion Sphere ORL
			$this->getMotifConsultationlso()->deleteMotifConsultationlso($id_cons);
			$this->getMotifConsultationlso()->addMotifConsultationlso($formData, $id_cons, $id_medecin);

			//insertion Reste Examen Clinique Lesion Sphere ORL
			$this->getResteExamenCliniquelso()->deleteResteExamenCliniquelso($id_cons);
			$this->getResteExamenCliniquelso()->addResteExamenCliniquelso($formData, $id_cons, $id_medecin);
		...
		if ($submit == 'rechercheVisualisation') {


			return $this->redirect()->toRoute('orl', array(
				'action' => 'espace-recherche-med'
			));
		} else {
			return $this->redirect()->toRoute('orl', array(
	...
	public function choixDossierAction()
	{
		...
		if ($sous_dossier == 1) {
			...
			return $this->redirect()->toUrl($this->baseUrl() . 'public/orl/tumeur-parotidienne?id_admission=' . $id_admission . '&id_cons=' . $id_cons);
		} else if ($sous_dossier == 4) {

			return $this->redirect()->toUrl($this->baseUrl() . 'public/orl/corps-etranger?id_admission=' . $id_admission . '&id_cons=' . $id_cons);
			//var_dump('corps etranger'); exit();

		} else if ($sous_dossier == 5) {
			return $this->redirect()->toUrl($this->baseUrl() . 'public/orl/cancer?id_admission=' . $id_admission . '&id_cons=' . $id_cons);
		}
	}
	...
	public function ficheObservationCliniqueAction()
	{
		...
		//Liste des examens Morphologiques
		$listeDesExamensMorphologiques = $this->demandeExamensTable()->getDemandeDesExamensMorphologiques();
		//var_dump($listeDesExamensMorphologiques);
		//exit();
		...
		$IdConsPremiere = $this->getMotifAdmissionTable()->getDonneesPremiereCons($id_pat, 1);
		...
		$antecedentOrl = $this->getAntecedentOrl()->getAntecedentsOrl($IdConsPremiere);
		if ($antecedentOrl) {
			$data['ant_chirurgicaux'] = $antecedentOrl['ant_chirurgicaux'];
			$data['antecedents_specifiques'] = $antecedentOrl['antecedents_specifiques'];
		}
		...
		//var_dump($IdConsPremiere); exit();

		$motif_admission = $this->getMotifAdmissionTable()->getMotifAdmission($IdConsPremiere);
		$nbMotif = $this->getMotifAdmissionTable()->nbMotifs($IdConsPremiere);

		//var_dump($motif_admission); exit();
		//POUR LES MOTIFS D'ADMISSION
		$k = 1;
		foreach ($motif_admission as $Motifs) {
			$data['motif_admission' . $k] = $Motifs['Libelle_motif'];
		...
		//modification pour l'histoire de la maladie
		//modification pour l'histoire de la maladie
		$histoireMaladie = $this->getHistoireMaladie()->getHistoireMaladie($IdConsPremiere);
		//var_dump($histoireMaladie); exit();
		if ($histoireMaladie) {
			$data['histoire_maladie'] = $histoireMaladie['histoire_maladie'];
		}
	...
	public function thyroideAction()
	{
		...
		//Nouvelle consultation
			...
			$IdConsPremiere = $this->getMotifAdmissionTable()->getDonneesPremiereCons($id_pat, 2);
			...
			//modification des antecedents du sous dossier thyroide
			$antecedentOrl = $this->getAntecedentOrl()->getAntecedentsOrl($IdConsPremiere);
			if ($antecedentOrl) {
				$data['ant_chirurgicaux'] = $antecedentOrl['ant_chirurgicaux'];
				$data['irradiation_cervical_anterieur'] = $antecedentOrl['irradiation_cervical_anterieur'];
			...
			//modification des motifs de consultation du sous dossier thyroide
			$motifHospitalisation = $this->getMotifHospitalisationOrl()->getMotifHospitalisation($IdConsPremiere);
			//var_dump($this->params()->fromPost('groupeIa'));exit();
			if ($motifHospitalisation) {
				$data['tumefaction_cervical_anterieur'] = $motifHospitalisation['tumefaction_cervical_anterieur'];
				$data['signes_thyroxicose'] = $motifHospitalisation['signes_thyroxicose'];
			...
			//Pour PEAU_CERVICAU_FACIALE
			//Pour PEAU_CERVICAU_FACIALE
			$peauCervicauFaciale = $this->getPeauCervicauFacialeOrl()->getPeauCervicauFaciale($IdConsPremiere);
			...
			//POUR GLANDE TYROIDE
			//POUR GLANDE TYROIDE
			$tyroide = $this->getTyroide()->getTyroide($IdConsPremiere);
			if ($tyroide) {
				$data['hypertrophie_globale'] = $tyroide['hypertrophie_globale'];
				$data['hypertrophie_localise'] = $tyroide['hypertrophie_localise'];
				$data['hypertrophie_nodulaire'] = $tyroide['hypertrophie_nodulaire'];
			...
			//POUR LES GROUPES GANGLIONNAIRES CERVICALES
			//POUR LES GROUPES GANGLIONNAIRES CERVICALES
			$groupeGanglionnaireCervicaux = $this->getGroupesGanglionnaires()->getGroupesGanglionnaires($IdConsPremiere);
			if ($groupeGanglionnaireCervicaux) {
				$data['groupeI'] = $groupeGanglionnaireCervicaux['groupeI'];
				$data['groupeIIa'] = $groupeGanglionnaireCervicaux['groupeIIa'];
				$data['groupeIIb'] = $groupeGanglionnaireCervicaux['groupeIIb'];
			...
			//Donnees de l'examen clinique
			$donneesExamenClinique = $this->getDonneesExamenClinique()->getDonneesExamenClinique($IdConsPremiere);
			if ($donneesExamenClinique) {
			...
			//POUR LES HORMONES TYROIDIENNES
			//POUR LES HORMONES TYROIDIENNES
			$hormonesTyroidiennes = $this->getHormonesTyroidiennesOrl()->getHormonesTyroidiennes($IdConsPremiere);
			if ($hormonesTyroidiennes) {
				$data['t4'] = $hormonesTyroidiennes['t4'];
				$data['tsh'] = $hormonesTyroidiennes['tsh'];
				$data['groupeIIIa'] = $hormonesTyroidiennes['groupeIIIa'];
			...
			$indicationsPreoperatoire = $this->getIndicationsOperatoireOrl()->getIndicationsOperatoiresOrl($IdConsPremiere);
			if ($indicationsPreoperatoire) {
				$data['indication_preoperatoire'] = $indicationsPreoperatoire['indication_preoperatoire'];
				$data['indication_peroperatoire'] = $indicationsPreoperatoire['indication_peroperatoire'];
			}
			$incidentAccidentPerOperatoire = $this->getIncidentAccidentPerOperatoire()->getIncidentAccidentPerOperatoireOrl($IdConsPremiere);
			if ($incidentAccidentPerOperatoire) {
				$data['incident_anesthesique'] = $incidentAccidentPerOperatoire['incident_anesthesique'];
				$data['incident_chirurgicaux'] = $incidentAccidentPerOperatoire['incident_chirurgicaux'];
				$data['groupeIVb'] = $incidentAccidentPerOperatoire['groupeIVb'];
			...
			$compteRenduOperatoireOrl = $this->getCompteRenduOperatoireOrl()->getCompteRenduOperatoireOrl($IdConsPremiere);
			if ($compteRenduOperatoireOrl) {
				$data['anesthesie'] = $compteRenduOperatoireOrl['anesthesie'];
				$data['incision'] = $compteRenduOperatoireOrl['incision'];
				$data['exploitation'] = $compteRenduOperatoireOrl['exploitation'];
			...
			$periodePostOperatoirePrecoce = $this->getPeriodePostOperatoirePrecoce()->getPeriodePostOperatoirePrecoce($IdConsPremiere);
			if ($periodePostOperatoirePrecoce) {
				$data['calcemie'] = $periodePostOperatoirePrecoce['calcemie'];
				$data['laryngectomie_indirecte'] = $periodePostOperatoirePrecoce['laryngectomie_indirecte'];
				$data['ablation_drain'] = $periodePostOperatoirePrecoce['ablation_drain'];
			...
			//Surveillance Tardive
			$surveillanceTardive = $this->getsurveillanceTardive()->getsurveillanceTardive($IdConsPremiere);
			//var_dump($surveillanceTardive);exit();
			if ($surveillanceTardive) {
				$data['plainte'] = $surveillanceTardive['plainte'];
				$data['qualite_voix_parlee'] = $surveillanceTardive['qualite_voix_parlee'];
			...
			$histologie = $this->getHistologie()->getHistologie($IdConsPremiere);
			if ($histologie) {
				$data['histologie'] = $histologie['histologie'];
			}
			...
			//POUR LA DEMANDE PRE-ANESTHESIQUE
			//POUR LA DEMANDE PRE-ANESTHESIQUE
			$donneesDemandeVPA = $this->getDemandeVisitePreanesthesiqueTable()->getDemandeVisitePreanesthesique($IdConsPremiere);
			...
			//POUR LES EXAMENS COMPLEMENTAIRES
			// DEMANDES DES EXAMENS COMPLEMENTAIRES
			$listeDemandesMorphologiques = $this->demandeExamensTable()->getDemandeExamensMorphologiques($IdConsPremiere);
			$listeDemandesBiologiques = $this->demandeExamensTable()->getDemandeExamensBiologiques($IdConsPremiere);
			...
			////RESULTATS DES EXAMENS BIOLOGIQUES DEJA EFFECTUES ET ENVOYER PAR LE BIOLOGISTE
			$listeDemandesBiologiquesEffectuerEnvoyer = $this->demandeExamensTable()->getDemandeExamensBiologiquesEffectuesEnvoyer($IdConsPremiere);
			$listeDemandesBiologiquesEffectuer = $this->demandeExamensTable()->getDemandeExamensBiologiquesEffectues($IdConsPremiere);
			...
			////RESULTATS DES EXAMENS MORPHOLOGIQUE
			$examen_morphologique = $this->getNotesExamensMorphologiquesTable()->getNotesExamensMorphologiques($IdConsPremiere);
			...
			////RESULTATS DES EXAMENS MORPHOLOGIQUES DEJA EFFECTUES ET ENVOYER PAR LE BIOLOGISTE
			$listeDemandesMorphologiquesEffectuer = $this->demandeExamensTable()->getDemandeExamensMorphologiquesEffectues($IdConsPremiere);


			// 	////RESULTATS DES EXAMENS BIOLOGIQUE
			$examen_biologique = $this->getNotesExamensBiologiquesTable()->getNotesExamensBiologiques($IdConsPremiere);
			...
			//DIAGNOSTICS
			//DIAGNOSTICS
			$infoDiagnostics = $this->getDiagnosticsTable()->getDiagnostics($IdConsPremiere);
			// POUR LES DIAGNOSTICS
			$k = 1;
			foreach ($infoDiagnostics as $diagnos) {
				$data['diagnostic' . $k] = $diagnos['libelle_diagnostics'];
			...
			// INSTANTIATION DE L'ORDONNANCE
			$infoOrdonnance = $this->getOrdonnanceTable()->getOrdonnanceNonHospi($IdConsPremiere);
			...
			/*CODES AJOUTES*/
			$rv_consultation = $this->getRvPatientConsTable()->getRendezVousConsultation($IdConsPremiere);
			$leRendezVous = $this->getRvPatientConsTable()->getInfosRendezVousConsultation($rv_consultation['ID_RV']);
	...
	//tumeur parotidienne action

	public function tumeurparotidienneAction()
	{
		...
		$form->get('id_patient')->setValue($id_pat);
		$form->get('id_admission')->setValue($id_admission);
		$form->get('sous_dossier')->setValue(3);
		...
		if ($consultation) {
			...
			//POUR Motif consultaion Tumeur
			//POUR Motif consultaion Tumeur
			$motifConsTum = $this->getMotifConsultationTumeur()->getMotifConsultationTumeur($id);
			if ($motifConsTum) {
				$data['tumefaction_parotidienne'] = $motifConsTum['tumefaction_parotidienne'];
				$data['paralysie_faciale_peripherique'] = $motifConsTum['paralysie_faciale_peripherique'];
				$data['adenopathie'] = $motifConsTum['adenopathie'];
			}

			//POUR Peau cervico faciale tumeur
			//POUR Peau cervico faciale tumeur
			$peauCF = $this->getPeauCervicoFacialeTumeur()->getPeauCervicoFacialeTumeur($id);
			if ($peauCF) {
				$data['peau_cervico_faciale'] = $peauCF['peau_cervico_faciale'];
				$data['autres_peau'] = $peauCF['autres_peau'];
			}


			//Glande parotidienne
			$glandeParotide = $this->getGlandeParotidienne()->getGlandeParotidienne($id);
			if ($glandeParotide) {
				$data['siege_parotidienne'] = $glandeParotide['siege_parotidienne'];
				$data['consistance_paro'] = $glandeParotide['consistance_paro'];
				$data['surface_paro'] = $glandeParotide['surface_paro'];
				$data['indolence_paro'] = $glandeParotide['indolence_paro'];
				$data['taille_mensuration'] = $glandeParotide['taille_mensuration'];
			}

			//Muqueuse parotidienne
			$muqueuseParotide = $this->getMuqueuseParotide()->getMuqueuseParotide($id);
			if ($muqueuseParotide) {
				$data['cavite_bucale'] = $muqueuseParotide['cavite_bucale'];
				$data['oropharynx'] = $muqueuseParotide['oropharynx'];
				$data['fosses_nasales'] = $muqueuseParotide['fosses_nasales'];
				$data['li'] = $muqueuseParotide['li'];
				$data['aires_ganglionnaires'] = $muqueuseParotide['aires_ganglionnaires'];
				$data['paires_cranienne'] = $muqueuseParotide['paires_cranienne'];
			}

			//PFP parotidienne
			$pfpParotide = $this->getPfpParotide()->getPfpParotide($id);
			if ($pfpParotide) {
				$data['paires_cranienne'] = $pfpParotide['paires_cranienne'];
			}

			//Otoscopie parotidienne
			$otoscopieParotide = $this->getOtoscopie()->getOtoscopie($id);
			if ($otoscopieParotide) {
				$data['otoscopie'] = $otoscopieParotide['otoscopie'];
			}


			//Examen para clinique tumeur parotidienne
			$examPara = $this->getExamenParacliniqueTumeur()->getExamenParacliniqueTumeur($id);
			if ($examPara) {
				$data['echographie_parotidienne'] = $examPara['echographie_parotidienne'];
				$data['citoponction'] = $examPara['citoponction'];
				$data['autres_examen_para'] = $examPara['autres_examen_para'];
			}


			//Operation tumeur parotidienne
			$opTumeurParo = $this->getOperationTumeurParotidienne()->getOperationTumeurParotidienne($id);
			if ($opTumeurParo) {
				$data['incidents_paro'] = $opTumeurParo['incidents_paro'];
				$data['cro_paro'] = $opTumeurParo['cro_paro'];
				$data['abords_paro'] = $opTumeurParo['abords_paro'];
				$data['decouvertes_paro'] = $opTumeurParo['decouvertes_paro'];
				$data['gestes_paro'] = $opTumeurParo['gestes_paro'];
				$data['fermeture_paro'] = $opTumeurParo['fermeture_paro'];
				$data['date_sortie'] = $opTumeurParo['date_sortie'];
				$data['suites_simples'] = $opTumeurParo['suites_simples'];
				$data['suites_compliquees'] = $opTumeurParo['suites_compliquees'];
				$data['groupeItum'] = $opTumeurParo['groupeItum'];
				$data['groupeIIatum'] = $opTumeurParo['groupeIIatum'];
				$data['groupeIIbtum'] = $opTumeurParo['groupeIIbtum'];
				$data['groupeIIItum'] = $opTumeurParo['groupeIIItum'];
			}
		...
		//Nouvelle consultation
			...
			$IdConsPremiere = $this->getMotifAdmissionTable()->getDonneesPremiereCons($id_pat, 3);
			...
			//POUR Motif consultaion Tumeur
			//POUR Motif consultaion Tumeur
			$motifConsTum = $this->getMotifConsultationTumeur()->getMotifConsultationTumeur($IdConsPremiere);
			if ($motifConsTum) {
				$data['tumefaction_parotidienne'] = $motifConsTum['tumefaction_parotidienne'];
				$data['paralysie_faciale_peripherique'] = $motifConsTum['paralysie_faciale_peripherique'];
				$data['adenopathie'] = $motifConsTum['adenopathie'];
			...
			//POUR Peau cervico faciale tumeur
			//POUR Peau cervico faciale tumeur
			$peauCF = $this->getPeauCervicoFacialeTumeur()->getPeauCervicoFacialeTumeur($IdConsPremiere);
			if ($peauCF) {
				$data['peau_cervico_faciale'] = $peauCF['peau_cervico_faciale'];
				$data['autres_peau'] = $peauCF['autres_peau'];
			}
			...
			//Glande parotidienne
			$glandeParotide = $this->getGlandeParotidienne()->getGlandeParotidienne($IdConsPremiere);
			if ($glandeParotide) {
				$data['siege_parotidienne'] = $glandeParotide['siege_parotidienne'];
				$data['consistance_paro'] = $glandeParotide['consistance_paro'];
				$data['surface_paro'] = $glandeParotide['surface_paro'];
			...
			//Muqueuse parotidienne
			$muqueuseParotide = $this->getMuqueuseParotide()->getMuqueuseParotide($IdConsPremiere);
			if ($muqueuseParotide) {
				$data['cavite_bucale'] = $muqueuseParotide['cavite_bucale'];
				$data['oropharynx'] = $muqueuseParotide['oropharynx'];
				$data['fosses_nasales'] = $muqueuseParotide['fosses_nasales'];
			...
			//PFP parotidienne
			$pfpParotide = $this->getPfpParotide()->getPfpParotide($IdConsPremiere);
			if ($pfpParotide) {
				$data['paires_cranienne'] = $pfpParotide['paires_cranienne'];
			}

			//Otoscopie parotidienne
			$otoscopieParotide = $this->getOtoscopie()->getOtoscopie($IdConsPremiere);
			if ($otoscopieParotide) {
				$data['otoscopie'] = $otoscopieParotide['otoscopie'];
			}
			...
			//Examen para clinique tumeur parotidienne
			$examPara = $this->getExamenParacliniqueTumeur()->getExamenParacliniqueTumeur($IdConsPremiere);
			if ($examPara) {
				$data['echographie_parotidienne'] = $examPara['echographie_parotidienne'];
				$data['citoponction'] = $examPara['citoponction'];
				$data['autres_examen_para'] = $examPara['autres_examen_para'];
			...
			//Operation tumeur parotidienne
			$opTumeurParo = $this->getOperationTumeurParotidienne()->getOperationTumeurParotidienne($IdConsPremiere);
			if ($opTumeurParo) {
				$data['incidents_paro'] = $opTumeurParo['incidents_paro'];
				$data['cro_paro'] = $opTumeurParo['cro_paro'];
				$data['abords_paro'] = $opTumeurParo['abords_paro'];
	...
	public function corpsEtrangerAction()
	{
		...
		$form->get('id_patient')->setValue($id_pat);
		$form->get('id_admission')->setValue($id_admission);
		$form->get('sous_dossier')->setValue(4);
		...
		if ($consultation) {
			...
			//POUR Motif consultaion Corps Etranger
			//POUR Motif consultaion Corps Etranger
			...
			//Examen Cliniquece
			$examenCliniquece = $this->getExamenCliniquece()->getExamenCliniquece($id);
			if ($examenCliniquece) {
				$data['duree_sejour'] = $examenCliniquece['duree_sejour'];
				$data['oreille'] = $examenCliniquece['oreille'];
				//$data['fosse_nasale'] = $examenCliniquece['fosse_nasale'];
				//$data['pharynx_corps'] = $examenCliniquece['pharynx_corps'];
				//$data['vri_corps'] = $examenCliniquece['vri_corps'];
				$data['autre_localisation'] = $examenCliniquece['autre_localisation'];
				$data['type_corps_etranger'] = $examenCliniquece['type_corps_etranger'];
				$data['nature_corps_etranger'] = $examenCliniquece['nature_corps_etranger'];
				$data['divers_corps'] = $examenCliniquece['divers_corps'];
			}

			//Examen Paraclinique
			$examenParacliniquece = $this->getExamenParacliniquece()->getExamenParacliniquece($id);
			if ($examenParacliniquece) {
				$data['mode_extraction'] = $examenParacliniquece['mode_extraction'];
			}



			//Operation corps etranger
			$operationCorpsEtranger = $this->getOperationCorpsEtranger()->getOperationCorpsEtranger($id);
			if ($operationCorpsEtranger) {
				$data['suites_simplece'] = $operationCorpsEtranger['suites_simplece'];
				$data['suite_compliqueesce'] = $operationCorpsEtranger['suite_compliqueesce'];
				$data['precision_suite'] = $operationCorpsEtranger['precision_suite'];
				$data['cro_ce'] = $operationCorpsEtranger['cro_ce'];
			}
		...
		//Nouvelle consultation
			...
			$IdConsPremiere = $this->getMotifAdmissionTable()->getDonneesPremiereCons($id_pat, 4);
			...
			//Examen Cliniquece
			$examenCliniquece = $this->getExamenCliniquece()->getExamenCliniquece($IdConsPremiere);
			if ($examenCliniquece) {
				$data['duree_sejour'] = $examenCliniquece['duree_sejour'];
				$data['oreille'] = $examenCliniquece['oreille'];
				//$data['fosse_nasale'] = $examenCliniquece['fosse_nasale'];
			...
			//Examen Paraclinique
			$examenParacliniquece = $this->getExamenParacliniquece()->getExamenParacliniquece($IdConsPremiere);
			if ($examenParacliniquece) {
				$data['mode_extraction'] = $examenParacliniquece['mode_extraction'];
			}
			...
			//Operation tumeur parotidienne
			$operationCorpsEtranger = $this->getOperationCorpsEtranger()->getOperationCorpsEtranger($IdConsPremiere);
			if ($operationCorpsEtranger) {
				$data['suites_simplece'] = $operationCorpsEtranger['suites_simplece'];
				$data['suite_compliqueesce'] = $operationCorpsEtranger['suite_compliqueesce'];
				$data['precision_suite'] = $operationCorpsEtranger['precision_suite'];
	...
	public function cancerAction()
	{
		...
		$form->get('id_patient')->setValue($id_pat);
		$form->get('id_admission')->setValue($id_admission);
		$form->get('sous_dossier')->setValue(5);
		...
		if ($consultation) {
			...
			//motif de consultation Lesion sphere ORL
			$moticonslso = $this->getMotifConsultationlso()->getMotifConsultationlso($id);
			if ($moticonslso) {
				$data['malade'] = $moticonslso['malade'];
				$data['date_apparitionC'] = $moticonslso['date_apparitionC'];
				$data['signes_fonctionnels_cancer'] = $moticonslso['signes_fonctionnels_cancer'];
				$data['adenopathie_cancer'] = $moticonslso['adenopathie_cancer'];
				$data['tracheotomie_cancer'] = $moticonslso['tracheotomie_cancer'];
				$data['decouverte_examen'] = $moticonslso['decouverte_examen'];
				$data['autres_motifLso'] = $moticonslso['autres_motifLso'];
			}



			//Examen clinique lesion sphere ORL
			$examcliniklso = $this->getExamenCliniquelso()->getExamenCliniquelso($id);
			if ($examcliniklso) {
				$data['peau_cervicoCan'] = $examcliniklso['peau_cervicoCan'];
				$data['otoscopieCan'] = $examcliniklso['otoscopieCan'];
				$data['cavite_bucaleCan'] = $examcliniklso['cavite_bucaleCan'];
				$data['fosses_nasalesCan'] = $examcliniklso['fosses_nasalesCan'];
				$data['liCan'] = $examcliniklso['liCan'];
				$data['reste_examen_cliniqueLso'] = $examcliniklso['reste_examen_cliniqueLso'];
				$data['aspect_cancer'] = $examcliniklso['aspect_cancer'];
				$data['zones_atteintes'] = $examcliniklso['zones_atteintes'];
				$data['margelle'] = $examcliniklso['margelle'];
				$data['larynx'] = $examcliniklso['larynx'];
				$data['hypopharynx'] = $examcliniklso['hypopharynx'];
				$data['vallecule'] = $examcliniklso['vallecule'];
				$data['mur_pharyngo_larynge'] = $examcliniklso['mur_pharyngo_larynge'];
				$data['oedeme'] = $examcliniklso['oedeme'];
				$data['superficiel_serpigineux'] = $examcliniklso['superficiel_serpigineux'];
				$data['bien_limite'] = $examcliniklso['bien_limite'];
				$data['keratosique'] = $examcliniklso['keratosique'];
				$data['cavumlso'] = $examcliniklso['cavumlso'];
				$data['oesophage_cervical'] = $examcliniklso['oesophage_cervical'];
				$data['aires_ganglionnaires_cancer'] = $examcliniklso['aires_ganglionnaires_cancer'];
				$data['tdm_rocherslso'] = $examcliniklso['tdm_rocherslso'];
			}



			//motif de consultation Lesion sphere ORL
			$resteexamlso = $this->getResteExamenCliniquelso()->getResteExamenCliniquelso($id);
			if ($resteexamlso) {
				$data['tdm_cervico_thoracique'] = $resteexamlso['tdm_cervico_thoracique'];
				$data['tdm_sinus_face'] = $resteexamlso['tdm_sinus_face'];
				$data['autres_result_exam'] = $resteexamlso['autres_result_exam'];
				$data['avis_rcp'] = $resteexamlso['avis_rcp'];
				$data['cro_lso'] = $resteexamlso['cro_lso'];
			}
		...
		//Nouvelle consultation
			...
			$IdConsPremiere = $this->getMotifAdmissionTable()->getDonneesPremiereCons($id_pat, 5);
			...
			//motif de consultation Lesion sphere ORL
			$moticonslso = $this->getMotifConsultationlso()->getMotifConsultationlso($IdConsPremiere);
			if ($moticonslso) {
				$data['malade'] = $moticonslso['malade'];
				$data['date_apparitionC'] = $moticonslso['date_apparitionC'];
				$data['signes_fonctionnels_cancer'] = $moticonslso['signes_fonctionnels_cancer'];
			...
			//Examen clinique lesion sphere ORL
			$examcliniklso = $this->getExamenCliniquelso()->getExamenCliniquelso($IdConsPremiere);
			if ($examcliniklso) {
				$data['peau_cervicoCan'] = $examcliniklso['peau_cervicoCan'];
				$data['otoscopieCan'] = $examcliniklso['otoscopieCan'];
				$data['cavite_bucaleCan'] = $examcliniklso['cavite_bucaleCan'];
			...
			//motif de consultation Lesion sphere ORL
			$resteexamlso = $this->getResteExamenCliniquelso()->getResteExamenCliniquelso($IdConsPremiere);
			if ($resteexamlso) {
				$data['tdm_cervico_thoracique'] = $resteexamlso['tdm_cervico_thoracique'];
				$data['tdm_sinus_face'] = $resteexamlso['tdm_sinus_face'];
				$data['autres_result_exam'] = $resteexamlso['autres_result_exam'];
	...
	*/
	...
	public function rechercheVisualisationTumeurParotidienneAction()
	{
	...
	*/
	...
	public function rechercheVisualisationCorpsEtrangerAction()
	{
	...
	*/
	...
	public function rechercheVisualisationCancerAction()
	{
	...
	*/
	...
	public function majFicheObservationCliniqueAction()
	{
		...
		if ($infoOrdonnance) {
			...
			//var_dump($listeMedicamentsPrescrits->current()); exit();
			$nbMedPrescrit = $listeMedicamentsPrescrits->count();
		} else {
			$nbMedPrescrit = null;
			$listeMedicamentsPrescrits = null;
	...
	{
		$id_pat = $this->params()->fromQuery('id_patient', 0);
		//$sous_dossier = (int) $this->params()->fromPost ('id_sous_dossier');
	...
	public function infoPatientRechercheAction()
	{
		...
		//$output = $this->getConsultationTable()->getListeConsultationPrecedentePatientAjax(886);

		// 	$output = $this->getConsultationTable()->getNbNoteMedicale(886, 2);
		///var_dump($output); exit();
