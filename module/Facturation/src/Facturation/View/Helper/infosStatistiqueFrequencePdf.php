...
class infosStatistiqueFrequencePdf extends fpdf
{
	...
	function Footer()
	{
		...
		$this->Cell(81, 5, 'Téléphone: 77 247 35 55 ', 0, 0, 'L', false);
	...
	function EnTetePage()
	{
		...
		$this->Image($tabURI[0] . 'public/img/logoorl.jpg', 162, 17, 45, 25);
	}
	...
	function CorpsDocument()
	{
		...
		if ($tabInformations) {
			...
			for ($i = 0; $i < count($tabInformations[0]); $i++) {

				$totalnbConsultations += $tabInformations[1][$tabInformations[0][$i]];
				$totalnbConsultationsUniques += array_key_exists($tabInformations[0][$i], $tabInformations[2]) ? $tabInformations[2][$tabInformations[0][$i]] : 0;



				if ($i % 2 == 0) {

					$nbConsultants = array_key_exists($tabInformations[0][$i], $tabInformations[2]) ? $tabInformations[2][$tabInformations[0][$i]] : 0;
					...
					$this->Cell(40, 7, iconv('UTF-8', 'windows-1252', $nbConsultants), 'BT', 0, 'L', 1);
					$this->Cell(40, 7, iconv('UTF-8', 'windows-1252', $tabInformations[1][$tabInformations[0][$i]]), 'BT', 0, 'R', 1);
			...
			$this->Cell(32, 7, 'TOTAL', 'BT', 0, 'L', 1);
			$this->SetFont('times', '', 13.3);
			$this->Cell(37, 7, $totalnbConsultationsUniques . ' ', 'BT', 0, 'L', 1);
			$this->SetFont('times', '', 13.3);
			$this->Cell(44, 7, $totalnbConsultations . ' ', 'BT', 0, 'R', 1);
		} else {
			echo  "<div align='center' style='font-size: 30px; font-family: times new roman;'> Aucune information à afficher </div>";
			exit();
		}
	}
