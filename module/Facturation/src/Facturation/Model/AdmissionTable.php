...
class AdmissionTable {
	...
	public function updateDemandeOperation( $donneesDemandeOperationOrl, $idDemandeOperation){
		$db = $this->tableGateway->getAdapter();
		$sql = new Sql($db);
		$sQuery = $sql->update()
		->table('demande_operation')
		->set( $donneesDemandeOperationOrl )
		->where(array('id_demande_operation' => $idDemandeOperation ));
		$sql->prepareStatementForSqlObject($sQuery)->execute();
	}
