...
class ResultatVisitePreanesthesiqueTable{
	...
	public function saveResultat($resultatVpa)
	{
		$data = array(
				...
				'dateVpa' =>(new DateHelper())->convertDateInAnglais( $resultatVpa->date_Vpa),
	...
	public function insererDemandeOperation($resultatVpa){
	    ...
	    ->columns($data)
	    ->values(array('id_cons' => $data['id_cons'] , 'numeroVPA' => $data['numeroVpa'], 'type_intervention' => $data['typeIntervention'], 'date_Vpa' => $data['dateVpa'], 'valeurAptitude' => $data['aptitude'], 'diagnostic'=>$data['diagnostic']));    
