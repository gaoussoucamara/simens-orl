...
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*--*-*-*-*-*-*-*-*-*--*-*-*-*--*-*-*-*-*-**--*-**-*--**-*-*-*-*-*-*-*-*-*-*-*-*-*--**-*-*-*-*-	
	...
	/***LORS DU CLICK SUR 'Terminer' ****/
	$("#terminer2, #terminer3, #terminer333").click(function() {
		if (valid == false){
	    ...
	    if($('#temoinVueRechercheViz').val() == 1){
	    	donnees["submit"] = 'rechercheVisualisation';
	    }
	 ...
	 // **********-- Donnees de l'examen clinique --*******
	    ...
	    donnees['reste_examen_cliniqueFo'] = $("#reste_examen_cliniqueFo").val();
	    donnees['examen_para_clinique'] = $("#examen_para_clinique").val();
	    donnees['peau_cervicoFo'] = $("#peau_cervicoFo").val();
	    donnees['otoscopieFo'] = $("#otoscopieFo").val();
	    donnees['cavite_bucaleFo'] = $("#cavite_bucaleFo").val();
	    ...
	    for(var i = 1 ; i < 10 ; i++ ){
	     	if($("#medicament_0"+i).val()){
	     		...
	     		donnees['formess_'+i] = $("#formess_"+i).val();
	    ...
	    donnees['date_intervenu'] = $("#date_intervenu").val();
	    donnees['date_sortie'] = $("#date_sortie").val();
	    donnees['aides'] = $("#aides").val();
	    donnees['operateur_aide'] = $("#operateur_aide").val();
		...
		donnees['anesthesiste'] = $("#anesthesiste").val();
		donnees['indication'] = $("#indication").val();
		donnees['protocole_operatoire'] = $("#protocole_operatoire").val();
		donnees['soins_post_operatoire'] = $("#soins_post_operatoire").val();
		donnees['surveillance'] = $("#surveillance").val();
		...
		// **=== SOUS DOSSIER TUMEUR PAROTIDIENNE
		
		// **=== motif_consultation_tumeur
		donnees['tumefaction_parotidienne'] = $("#tumefaction_parotidienne").val();
		donnees['paralysie_faciale_peripherique'] = $("#paralysie_faciale_peripherique").val();
		donnees['adenopathie'] = $("#adenopathie").val();
		//donnees['autresMotifConsTumeur'] = $("#autresMotifConsTumeur").val();	
		
		// **=== motif_consultation_tumeur
		
		donnees['peau_cervico_faciale'] = $("#peau_cervico_faciale").val();
		donnees['autres_peau'] = $("#autres_peau").val();
		
		// **=== Glande parotidienne
		donnees['siege_parotidienne'] = $("#siege_parotidienne").val();
		donnees['consistance_paro'] = $("#consistance_paro").val();
		donnees['surface_paro'] = $("#surface_paro").val();
		donnees['indolence_paro'] = $("#indolence_paro").val();
		donnees['taille_mensuration'] = $("#taille_mensuration").val();
		
		// **=== Muqueuse parotidienne
		donnees['cavite_bucale'] = $("#cavite_bucale").val();
		donnees['oropharynx'] = $("#oropharynx").val();
		donnees['fosses_nasales'] = $("#fosses_nasales").val();
		donnees['li'] = $("#li").val();
		donnees['aires_ganglionnaires'] = $("#aires_ganglionnaires").val();
		donnees['paires_cranienne'] = $("#paires_cranienne").val();
		...
		donnees['otoscopie'] = $("#otoscopie").val();
		
		// **=== Examen para clinique tumeur parotidienne
		donnees['echographie_parotidienne'] = $("#echographie_parotidienne").val();
		donnees['citoponction'] = $("#citoponction").val();
		donnees['autres_examen_para'] = $("#autres_examen_para").val();
		...
		donnees['groupeItum'] = $("#groupeItum:checked").val(); if(!donnees['groupeItum']){ donnees['groupeItum'] = 0;}
		donnees['groupeIIatum'] = $("#groupeIIatum:checked").val(); if(!donnees['groupeIIatum']){ donnees['groupeIIatum'] = 0;}
		donnees['groupeIIbtum'] = $("#groupeIIbtum:checked").val(); if(!donnees['groupeIIbtum']){ donnees['groupeIIbtum'] = 0;}
		donnees['groupeIIItum'] = $("#groupeIIItum").val();
	   ...
	   //  Sous Dossier Corps Etranger
		...
		donnees['divers_corps'] = $("#divers_corps").val();
		
		// **=== Examen Paralinique Corps Etranger
		donnees['mode_extraction'] = $("#mode_extraction").val();
		
		// **=== Operation corps etranger
		donnees['suites_simplece'] = $("#suites_simplece").val();
		donnees['suite_compliqueesce'] = $("#suite_compliqueesce").val();
		donnees['precision_suite'] = $("#precision_suite").val();
		donnees['cro_ce'] = $("#cro_ce").val();
		
		    //  Sous Dossier Lesion Sphere ORL
		   //   Sous Dossier Lesion Sphere ORL
		
		// **=== Motif consultation Lesion Sphere ORL
		donnees['malade'] = $("#malade").val();
		donnees['date_apparitionC'] = $("#date_apparitionC").val();
		donnees['signes_fonctionnels_cancer'] = $("#signes_fonctionnels_cancer").val();
		donnees['adenopathie_cancer'] = $("#adenopathie_cancer").val();
		donnees['tracheotomie_cancer'] = $("#tracheotomie_cancer").val();
		donnees['decouverte_examen'] = $("#decouverte_examen").val();
		donnees['autres_motifLso'] = $("#autres_motifLso").val();
		
		// **=== Examen Clinique Lesion Sphere ORL
		donnees['peau_cervicoCan'] = $("#peau_cervicoCan").val();
		donnees['otoscopieCan'] = $("#otoscopieCan").val();
		donnees['cavite_bucaleCan'] = $("#cavite_bucaleCan").val();
		donnees['fosses_nasalesCan'] = $("#fosses_nasalesCan").val();
		donnees['liCan'] = $("#liCan").val();
		donnees['reste_examen_cliniqueLso'] = $("#reste_examen_cliniqueLso").val();
		donnees['aspect_cancer'] = $("#aspect_cancer").val();
		donnees['zones_atteintes'] = $("#zones_atteintes").val();
		donnees['margelle'] = $("#margelle").val();
		donnees['larynx'] = $("#larynx").val();
		donnees['hypopharynx'] = $("#hypopharynx").val();
		donnees['vallecule'] = $("#vallecule").val();
		donnees['mur_pharyngo_larynge'] = $("#mur_pharyngo_larynge").val();
		donnees['oedeme'] = $("#oedeme:checked").val(); if(!donnees['oedeme']){ donnees['oedeme'] = 0;}
		...
		// **=== Resultat Examen paraclinique Lesion Sphere ORL
		donnees['tdm_cervico_thoracique'] = $("#tdm_cervico_thoracique").val();
		donnees['tdm_sinus_face'] = $("#tdm_sinus_face").val();
		donnees['autres_result_exam'] = $("#autres_result_exam").val();
		donnees['avis_rcp'] = $("#avis_rcp").val();
		donnees['cro_lso'] = $("#cro_lso").val();
