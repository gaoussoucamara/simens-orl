function chargerMedicaments(myArrayMedicament, myArrayForme, myArrayBoite, myArrayTypeQuantiteMedicament){
...
$(function(){
  ...
  $( "LesMedicaments khassBoite input" ).autocomplete({
	  source: myArrayBoite
  });
...
function creerLalisteMedicament ($listeDesElements, $ListeForme, $ListeBoite, $ListeQuantite) {
    	var index = $("LesMedicaments").length; 
			        $liste = "<div id='Medicament_"+(index+1)+"'>"+
                             ...
                             "<th id='SelectMedicament_"+(index+1)+"' style='width: 20%;'>";
                    ...
                    $liste +="</th>"+
                             ...
                             "<th id='noteMedicament_"+(index+1)+"' style='width: 20%;'  >"+
                             ...
                             "<th id='noteMedicamentss_"+(index+1)+"' style='width: 18%;'  >"+
                             "<khassBoite><input type='text' id='formess_"+(index+1)+"' name='formess_"+(index+1)+"' style='width: 100%; margin-top: 3px; height: 30px; margin-bottom: 0px; font-size: 15px; padding-left: 10px;' > </khassBoite>" +
                             "</th >"+
                    ...
                    chargerMedicaments($listeDesElements, $ListeForme, $ListeBoite, $ListeQuantite);
...
function partDefautMedicament (Liste, ListeForme, ListeBoite, ListeQuantite, n) { 
	var i = 0;
	for( i ; i < n ; i++){
		creerLalisteMedicament(Liste, ListeForme, ListeBoite, ListeQuantite);
	}
	if(n == 1){
		$(".supprimerMedicament" ).replaceWith(
				"<img class='supprimerMedicament' style='margin-left: 5px; margin-top: 10px;' src='../images/images/sup2.png' />"
...
function supprimer_medicament_selectionne(id) { 
	...
	for(var i = (id+1); i <= nbListeMedicaments(); i++ ){
		...
		var notess = $('#noteMedicamentss_'+i+' input').val();
		$("#noteMedicamentss_"+(i-1)+" input").val(notess);
...
function vider_medicament_selectionne(id) {
	...
	$("#noteMedicamentss_"+id+" input").val("");
...
//	        success: function(data) {
		        ...
		        for(var i = 1; i <= nbListeMedicaments(); i++ ){
	    			...
	    			$("#noteMedicamentss_"+i+" input").attr('disabled',true); $("#noteMedicamentss_"+i+" input").css({'background':'#f8f8f8'});
...
//		return false;
	...
	$("#ordonnance").click(function(){ 
		for(var i = 1; i <= nbListeMedicaments(); i++ ){
			...
			$("#noteMedicamentss_"+i+" input").attr('disabled',false); $("#noteMedicamentss_"+i+" input").css({'background':'white'});
